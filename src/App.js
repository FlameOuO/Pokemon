...
const pokemonReducer = (state, action) => {
	...
	const MAX_HUNGER = 100; //100不餓 0餓Petshopcontext
	const MAX_MOOD = 100;
	const MIN_HEALTH = 0;
	const MIN_HUNGER = 0;
	const MIN_MOOD = 0;
	...
	switch (action.type) {
			...
			return state.map((p) =>
				p.name === action.name
					? {
							...p,
							hunger: Math.min(
								p.hunger + FEED_INCREMENT,
								MAX_HUNGER
							),
							health: Math.min(
								p.health + HEALTH_INCREMENT_FEED,
								MAX_HEALTH
							),
					  }
					: p
			...
			return state.map((p) =>
				p.name === action.name
					? {
							...p,
							health: Math.min(
								p.health + HEALTH_INCREMENT_WATER,
								MAX_HEALTH
							),
					  }
					: p
			...
			return state.map((p) =>
				p.name === action.name
					? {
							...p,
							mood: Math.min(
								p.mood + MOOD_INCREMENT_PLAY,
								MAX_MOOD
							),
							hunger: Math.max(
								p.hunger - HUNGER_DECREMENT_PLAY,
								MIN_MOOD
							),
					  }
					: p
		...
		case 'UPDATE_TIME':
			return state.map((p) => ({
				...
				hunger: Math.min(
					Math.max(p.hunger - 5, MIN_HUNGER),
					MAX_HUNGER
				),
...
function App() {
	...
	useEffect(() => {
		if (selectedPokemon) {
			const updatedSelected = pokemon.find(
